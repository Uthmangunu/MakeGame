...
exports.generateGameFromStory = void 0;
...
// const gameSchema = { ... };
exports.generateGameFromStory = functions.https.onCall(async (data, context) => {
    // 1. Auth Check
    if (!context.auth) {
    ...
    try {
        ...
        // Save Meta
        await db.collection('games').doc(gameId).set(Object.assign(Object.assign({}, gameData.gameMeta), { gameId, ownerUserId: userId, createdAt: admin.firestore.FieldValue.serverTimestamp() }));
        ...
        if (gameData.gameMeta.characters && gameData.gameMeta.characters.length > 0) {
            ...
            (async () => {
                try {
                    const { generateSprites } = await Promise.resolve().then(() => require('./generateSprites'));
